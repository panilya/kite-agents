...
/**
 * End-to-end smoke tests against the real OpenAI Responses API. Gated on {@code OPENAI_API_KEY}
 * and the {@code live} JUnit tag — run via {@code ./gradlew :kite-openai:liveTest}, skipped by the
 * default {@code test} task. Uses the cheapest model. Assertions are shallow: we verify the round
 * trip works and the shape is what we expect, not the model's semantic output.
 */
@Tag("live")
...
class OpenAiLiveSmokeTest {
    ...
    void text() {
        try (var kite = Kite.builder()
                .provider(new OpenAiProvider(System.getenv("OPENAI_API_KEY")))
                .tracing(Tracing.off())
                .build()) {
