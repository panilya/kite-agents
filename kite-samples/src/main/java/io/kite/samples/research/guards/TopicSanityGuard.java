        ...
        return Guard.<T>input("topic-sanity")
                ...
                .check(in -> {
                    ...
                    if (text == null || text.isBlank()) {
                        return GuardDecision.block("Please provide a research question. An empty query can't be researched.");
                    }
                    String trimmed = text.trim();
                    if (trimmed.length() < 6) {
                        return GuardDecision.block("Question is too short to research (got '" + trimmed
                                + "'). Please describe the topic in a full sentence.");
                    }
                    if (!trimmed.contains(" ")) {
                        return GuardDecision.block("Single-word queries aren't useful for deep research. "
